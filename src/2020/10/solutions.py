...
def backsearch(change_map):
    return change_map[-1] == 0 or sum(
        backsearch(change_map[: -1 - i]) for i in range(change_map[-1])
    )
...
def part_2(input_data: str):
    ...
    adapters = [int(n) for n in input_data.splitlines()] + [0]
    adapters.append(max(adapters) + 3)
    change_map = tuple(sum(1 <= i - j <= 3 for j in adapters) for i in sorted(adapters))
